...
const helpers = {
  ...
  url: function(options) {
    ...
    function compile(components) {
      ...
      view_url = findViewByUrl(components.view)[components.lang];
      let parameters = [];
      for (component_index in Object.keys(components)) {
        ...
        //TODO: Fix component name from filters?
        parameters.push(component+"="+components[component])
      }
      let url = "/"+components.lang+"/"+view_url+(parameters.length > 0 ? "?"+parameters.join("&") : "");
      return url;
    }
  ...
  get_type_url: function(type,classification) {
    ...
    switch(type) {
      ...
      case "state": return "regiones";
      case "areas": return "regiones";
