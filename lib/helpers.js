...
const helpers = {
  ...
  lang: function(options) {
    ...
    const default_lang = options.data.root.general.default_lang[0].es;
  ...
  url: function(options) {
      ...
      for (component_index in Object.keys(components)) {
        ...
        if (component.slice(0,1)=="_") {
          ...
          if (component == "_anchor") {
            ...
            view_url = view_url.replace(":"+component.slice(1),components[component]);
          }
        }
        else {
    ...
    let url_components = extract(options.data.root.current_url);
  ...
  format_amount: function(value, currency) {
    if (value) {
  ...
  get_classification_type: function(classification) {
    ...
    switch (classification) {
      ...
      case "funcionario": return "person";
      case "proveedor": return "person";
      case "owner": return "person";
      case "contract": return "contract";
      case "company": return "company";
      case "banco": return "company";
      case "institution": return "institution";
      case "dependencia": return "institution";
      case "unidad-compradora": return "institution";
      case "city": return "area";
      case "municipality": return "area";
      case "state": return "area";
      case "country": return "area";
      default:
        console.error("resultIsType: Unknown classification:",classification);
        return false;
    }
  },  
  ...
  get_classification_icon: function(classification,subclassification,govLevel) {
    if (constants.classification_icons[classification] && constants.classification_icons[classification][subclassification] && constants.classification_icons[classification][subclassification][govLevel]) {
      // console.log("get_classification_name found",type,role_names[type][directionality].label);
  ...
  get_type_plural: function(type, short=false,options) {
    if (constants.type_names[type]) {
      ...
      return helpers._("Entidades",options);
    }
  },
  ...
  get_type_url: function(type,classification) {    
    if (!type) {
    ...
    switch(type) {
      ...
      default: console.log("get_type_url unknown",type,classification); return "unknown";
    }
  },
  ...
  get_first_year: function(yearSummary) {
    return Object.keys(yearSummary)[0];
  },
  get_last_year: function(yearSummary) {
    return Object.keys(yearSummary)[Object.keys(yearSummary).length -1];
  },
}
