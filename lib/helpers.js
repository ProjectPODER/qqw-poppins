...
const helpers = {
  ...
  format_number: function(value) { 
    if (value || value === 0) {
      nt = helpers._number_to_text(value);
       const formatted = nt.value.toLocaleString('es-MX', {style: "decimal", maximumFractionDigits: 1});
  ...
  cobranding: function(sources) {
    // console.log("cobranding",sources)

    for (s in sources) {
      if (constants.cobranding_sources.hasOwnProperty(sources[s].id)) {

        return constants.cobranding_sources[sources[s].id];
      }
    }
