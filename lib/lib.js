    ...
    if (filter.collections.indexOf(collection) > -1 || filter.collections.indexOf("all") > -1 ) {
  	...
  	if (queryValue && filterApplies) {
        ...
        if (filter.type == "minmax") {
          ...
          if (value[0] == ">") { delete value[0] }
          if (value[1] == "<") { delete value[1] }
  			}
        if (filter.type == "date") {
          let min = (query[filter.htmlFieldName+"-min"] || "1900-01-01 00:00:01").trim();
          let max = (query[filter.htmlFieldName+"-max"] || "9999-12-31 23:59:59").trim();
          // console.log("getFilters date",min,max);
          value = [
            ">" + (new Date( min ).toISOString()).replace("Z",""),
            "<" + (new Date( max ).toISOString()).replace("Z",""),
          ]
...
function cleanField(value,previousValue) {
  ...
  if (cleanField.type == "date") {
    ...
    if (previousValue) {
      cleanField.value = {
        min: moment(previousValue.value.substr(1)).format("YYYY-MM-DD"),
        max: moment(cleanField.value.substr(1)).format("YYYY-MM-DD")
      }
    }
  }
...
function redirectToLanguage(es,en) {
  return catchError(async function(req, res, next) {
    ...
    let current_lang = res.locals.current_lang || req.app.locals.general.current_lang[0].es;
    let newURL = "";
    if (current_lang == "es") {
      newURL = "/es"+es;
    }
...
function staticPage(templateName, layout="layout") {
  return catchError(async function(req, res, next) {
    console.log("staticPage","res.locals",res.locals);
