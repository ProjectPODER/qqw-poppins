...
function loadSettings(app,callback) {
  ...
  appLocalsFromCSV(app,"general",process.env.CSVSETTINGS_GENERAL_URL,["id","es","en"], function() {
    appLocalsFromCSV(app,"buscadores",process.env.CSVSETTINGS_BUSCADORES_URL,["tipo-buscador","id"], function() {
      appLocalsFromCSV(app,"notas",process.env.CSVSETTINGS_NOTAS_URL,["id","url","titulo","fecha","medio","autor","explicacion_es","explicacion_en"], function() {
        callback(app.locals);
      });
    });
  });
...
function reloadSettingsView(collection,templateName,idFieldName) {
  return catchError(async function(req, res, next) {
    loadSettings(res,function(appLocals) {
      res.json({
...
//TODO: Cache requests
async function initAPIFilter(filter, filters, res, debug) {
  ...
  //Get data from api
  rawOptions = await getAPI({},localFilter.api_request.collection,localFilter.api_request.filters,debug);
...
async function cleanFilters(filters, collection, res, debug) {            
  ...
  for (filterElement in allFilters) {
    ...
    if (filter.api_request) {
      filter = await initAPIFilter(filter, filters, res, debug);
    }
...
function searchPage2020(defaultFilters, templateName) {
  return catchError(async function(req, res, next) {
    ...
    if (!filters.error) {
      ...
      // console.log("If no filters, ask for default results",filters)
      if (Object.keys(filters).length == 0 || (Object.keys(filters).length == 1 && filters.collection=="all" )) {
    ...
    const filters_cleaned = await cleanFilters(filters, collection, res, debug);

    const title = generateSearchTitle(filters_cleaned,res);

    const share_url = req.originalUrl.replace(/\&/g,"%26");
...
function generateSearchTitle(filters,res) {
  ...
  for (f in filters) {
    ...
    if (filter.apiFilterName == "collection") {
      title.collection = helpers.get_type_plural(filter.value,true,res.locals.lang);
    }
    else {
      let filterValue;
      if (!filter.hidden) {
  ...
  title.full = title.collection + (title.filters.length ? " - " + title.filters.join(" - ")  : "") + " - " +  title.brand;
  return title;
}
...
function entityPage(collection,templateName,idFieldName) {
  return catchError(async function(req, res, next) {
    ...
    let metaTitle = "";
    metaTitle+=result.data[0].name || result.data[0].title || (result.data[0].records ? result.data[0].records.compiledRelease.contracts[0].title : result.data[0].contracts.title);
    metaTitle+=" - ";
...
function redirectToSearch(page) {
  return catchError(async function(req, res, next) {
    ...
    let current_lang = req.params.lang || (req.app.locals.general.default_lang ? req.app.locals.general.default_lang[0].es : "es");
    ...
    .replace("/unidades-compradoras\?","/buscador/?collection=institutions&subtipo-institucion=unidad-compradora&")
...
//   return catchError(async function(req, res, next) {
//     console.log(req);
//     return res.redirect("/es/sadico"+res.locals.general.products_landing);
//   })
// }
