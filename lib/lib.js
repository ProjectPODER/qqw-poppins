...
function cleanField(value) {
  ...
  if (cleanField.type == "minmax") {
    if (value.min || value.max) {
      cleanField.value = {}
      cleanField.value.min = decodeURIComponent(decodeURIComponent(value.min)),
      cleanField.value.max = decodeURIComponent(decodeURIComponent(value.max))
    }
  }
...
function cleanFilters(filters) {
  const cleanFilters = clone(constants.filter_elements);

  for (filterElement in cleanFilters) {
    let filter = cleanFilters[filterElement];
