...
async function initAPIFilter(filter, filters, res, debug) {
  ...
  if (localFilter.api_request.filters) {
    Object.keys(localFilter.api_request.filters).map(  f => { if (localFilter.api_request.filters[f].indexOf("#ref/") == 0 ) { localFilter.api_request.filters[f] = filters[localFilter.api_request.filters[f].substring(5)] } } )
  }
    ...
    if (filter.apiField == "_collection") {
      ...
      if (!filter.hidden) {
        if (filter.type=="select") {
          ...
          if (filter.options) {
            title+= " - " + filter.fieldLabel + ": " + filter.options[(filter.value || filter.default.value)];
          }
          else {
            title+="Uninitialized "+ filter.fieldLabel
          }

        }
        else if (filter.type=="minmax") {
          // console.log("generateSearchTitle",filter.fieldLabel,filter.value);
          title+= " - " + filter.fieldLabel + ": " + (filter.value ? filter.value.min+" a "+filter.value.max : filter.default.value);
        }
        else {
          title+= " - " + filter.fieldLabel + ": " + (filter.value || filter.default.value);
        }
      }
    }
...
//       let supplier_object = cloneDeep(supplier_template);
