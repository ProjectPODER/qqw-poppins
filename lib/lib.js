...
//TODO: Cache requests
async function initAPIFilter(filter, filters, debug) {
  ...
  if (rawOptions.data.length > 1) {
    ...
    rawOptions.data.map(option => { filter.options[option.id] = helpers.translate_area(option.name) })
  }
  else {
...
async function cleanFilters(filters, collection, debug) {            
  ...
  for (filterElement in allFilters) {
    ...
    if (filter.api_request) {
      filter = await initAPIFilter(filter, filters, debug);
    }
...
function searchPage2020(defaultFilters, templateName) {
  return catchError(async function(req, res, next) {
    ...
    const filters_cleaned = await cleanFilters(filters, collection, debug);
  ...
  if (result.memberships) {
    ...
    if (result.memberships.length > 0) {
      for (m in result.memberships) {
        ...
        memberships[role].memberships.push(result.memberships[m]);
      }
