...
async function getAPI(req,collection,filters,debug) {
  ...
  //Delete collection filter from api requests
  delete filters["_collection"];
...
function getFilters(query, defaultFilters) {
  ...
  for (filterElement in constants.filter_elements) {
    // console.log("getFilters 2",constants.filter_elements[filterElement].htmlFieldName,query[constants.filter_elements[filterElement].htmlFieldName],constants.filter_elements[filterElement].collections);
  	...
  	if (
      (
        query[ constants.filter_elements[filterElement].htmlFieldName ]
        || constants.filter_elements[filterElement].type === "bool")
    ) {
  		for (apiField in constants.filter_elements[filterElement].apiFieldNames) {
  			// console.log(constants.filter_elements[filterElement].apiFieldNames[apiField],constants.filter_elements[filterElement].htmlFieldName);
        const apiFieldName = constants.filter_elements[filterElement].apiFieldNames[apiField];
  			let value = encodeURIComponent(query[constants.filter_elements[filterElement].htmlFieldName].trim());

        if (constants.filter_elements[filterElement].type == "string") {
  				value = "/"+value+"/i"
  			}
        if (constants.filter_elements[filterElement].type == "date") {
  				value = (new Date(value).toISOString()).replace("Z","");
  			}
        if (constants.filter_elements[filterElement].type == "bool") {
          if (value == "true") {
            value = "";
          }
          else {
        ...
        if (constants.filter_elements[filterElement].modifier) {
          value = constants.filter_elements[filterElement].modifier+value;
        }
        if (constants.filter_elements[filterElement].repeated) {
...
function cleanField(value) {
  let cleanField = constants.filter_elements[filterElement];
  cleanField.apiField = constants.filter_elements[filterElement].apiFieldNames[apiField];
...
function cleanFilters(filters) {
  ...
  for (filterElement in constants.filter_elements) {
  	for (apiField in constants.filter_elements[filterElement].apiFieldNames) {
      const value = filters[constants.filter_elements[filterElement].apiFieldNames[apiField]];
      const hasValue = filters.hasOwnProperty(constants.filter_elements[filterElement].apiFieldNames[apiField]);
    	if (value || (constants.filter_elements[filterElement].type == "bool" && hasValue)) {
        cleanFilters.filters_active = {
           hidden: true,
           value: true
         };
        ...
        if (typeof value == "object") {
          // console.log("cleanFilters array",constants.filter_elements[filterElement],value);
          for (valueItem in value) {
            // console.log("cleanFilters",constants.filter_elements[filterElement].htmlFieldName,value[valueItem],constants.filter_elements[filterElement].modifier);
            if (constants.filter_elements[filterElement].modifier) {
              if (value[valueItem].indexOf(constants.filter_elements[filterElement].modifier) == 0) {
                cleanFilters[constants.filter_elements[filterElement].htmlFieldName] = cleanField(value[valueItem]);
              }
            }
            else {
              cleanFilters[constants.filter_elements[filterElement].htmlFieldName] = cleanField(value[valueItem]);
    	    ...
    	    cleanFilters[constants.filter_elements[filterElement].htmlFieldName] = cleanField(value);
        }
    	}
  	}
  }
...
function searchPage2020(defaultFilters) {
  return catchError(async function(req, res, next) {
    ...
    const result = await getAPI(req, filters._collection || "autocomplete", filters, debug);
    ...
    res.render("searcher", {
      collection: filters._collection || "all",
