...
async function getAPI(req,collection,filters,debug) {
  ...
  params["products"] = req.app.locals.general.products_active[0][helpers.env()];
...
function entityPage(collection,templateName,idFieldName) {
  return catchError(async function(req, res, next) {
    ...
    metaTitle+=result.data[0].name || result.data[0].title || (result.data[0].records ? result.data[0].records.compiledRelease.contracts[0].title : result.data[0].contracts ? result.data[0].contracts.title : "Sin título");
    metaTitle+=" - ";
...
function redirectToSite() {
  return catchError(async function(req, res, next) {
    console.log("redirectoToSite",req.headers.host);
    if (req.headers.host=="saluddineroycorrupcion.quienesquien.wiki") {
      redirectToLanguage("/salud","/salud")(req, res, next);
    }
    else {
