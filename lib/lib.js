...
function getFilters(query, collection, defaultFilters) {
  ...
  for (filterElement in constants.filter_elements) {
    ...
    if (filter.collections.indexOf(collection) > -1 || filter.collections.indexOf("all") > -1 ) {
  	...
  	if (queryValue && filterApplies) {
  		for (apiField in filter.apiFilterName) {
...
function cleanField(value,previousValue) {
  ...
  if (cleanField.type == "minmax") {
    cleanField.value = {
      min: decodeURIComponent(decodeURIComponent(value.min)),
      max: decodeURIComponent(decodeURIComponent(value.max))
    }
  //   console.log("minmax",previousValue)
  //   //TODO: No funciona cuando hay un solo valor
  //   if (previousValue) {
  //   }
  }
  if (cleanField.type == "date") {
    console.log("date",previousValue);
    //TODO: No funciona cuando hay un solo valor
    if (previousValue) {
      cleanField.value = {
        min: moment(value.min).format("YYYY-MM-DD"),
        max: moment(value.max).format("YYYY-MM-DD")
      }
    }
  }


  if (cleanField.apiField == "sort") {
    //TODO: Sort direction
...
function cleanFilters(filters) {
  ...
  for (filterElement in constants.filter_elements) {
    ...
    let value = filters[filter.apiFilterName];
    if (filter.type=="minmax") {
      value = {
        min: filters[filter.apiFilterName.min],
        max: filters[filter.apiFilterName.max]
      }
    }
    const hasValue = filters.hasOwnProperty(filter.apiFilterName);
...
function searchPage2020(defaultFilters, templateName) {
  return catchError(async function(req, res, next) {
    ...
    // TODO: Get collection before filters
    const collection = getCollection(req.query);
    // TODO: Only apply filters for current collection
    ...
    const filters_cleaned = cleanFilters(filters);
...
function generateSearchTitle(filters) {
  ...
  for (f in filters) {
    ...
    if (filter.apiField == "_collection") {
      ...
      if (!filter.hidden && filter.apiFieldNames[0] != "sort") {
...
function entityPage(collection,templateName,idFieldName) {
  return catchError(async function(req, res, next) {
    ...
    let metaTitle = "";
    if (collection == "contracts"){ metaTitle = result.data[0].records[0].contracts[0].title + " < Contrato < QuiénEsQuién.Wiki"}
    if (collection == "persons"){ metaTitle = result.data[0].name + " < Persona < QuiénEsQuién.Wiki" }
    if (collection == "institutions"){ metaTitle = result.data[0].name + " < Institución Pública < QuiénEsQuién.Wiki" }
    if (collection == "companies"){ metaTitle = result.data[0].name + " < Empresa < QuiénEsQuién.Wiki" }
    if (collection == "countries"){ metaTitle = result.data[0].name  + " < Mujeres en la bolsa < País < QuiénEsQuién.Wiki" }
