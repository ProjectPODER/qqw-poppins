	...
	for (page=0; page <= pages_count; page++) {		
		...
		if (page > (current_page-3) && page < (current_page+3) ) {
			...
			if (page == current_page) {
				pagesArray.push({ type: "current", value: current_page, enabled: false })		
			}
			else {
				pagesArray.push({ type: "page", value: page, enabled: true })		
			}
		}	
		//If we have more than 5 pages to go, add link next 5 pages
		if (page == current_page+5) {
			pagesArray.push({ type: "next5", value: page, enabled: true })					
		}
		//If we have more than 10 pages to go, add link next 10 pages
		if (page == current_page+10) {
			pagesArray.push({ type: "next10", value: page, enabled: true })					
		}
	}

	//If we are not at the last page, add link to last page
	if (current_page != pages_count) {
		pagesArray.push({ type: "last", value: pages_count, enabled: true })								
	}

	//Next page is linked if we're not at last page
	pagesArray.push({ type: "next", value: current_page+1, enabled: (current_page < pages_count) })
